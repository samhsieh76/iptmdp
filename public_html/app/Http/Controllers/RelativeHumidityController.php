...
class RelativeHumidityController extends Controller {
    /**
    ...
    public function __construct() {
        $this->program_name =  "relative_humidity_sensors";
    ...
    public function index() {
        return view('backend.sensors.relative_humidity.index');
    }
    ...
    public function getResources(Request $request, $toilet_id) {
        ...
        $records = TemperatureSensor::select('*')
            ->select('id', 'name', 'sensor_id', 'latest_humidity_raw_data as latest_raw_data', 'latest_humidity_value as latest_value', 'latest_humidity_updated_at as latest_updated_at')
            ->Search($searchValue)
            ->skip($start)
            ->take($rowperpage)
    ...
    public function show($toilet_id, $id) {
        ...
        return view('backend.sensors.relative_humidity.show', compact('sensor', 'toilet', 'fetchLocations'));
    }
    ...
    public function store(TemperatureRequest $request, $toilet_id) {
        ...
        if ($res->save()) {
            ...
            return ErrorCodeUtils::jsonResponse(ErrorCodeUtils::SUCCESS, ['messages' => trans('messages.relative_humidity_sensor') . $res->name . trans('messages.successfully_added')]);
        }
        return ErrorCodeUtils::jsonResponse(ErrorCodeUtils::FAILURE, null, trans('messages.relative_humidity_sensor') . trans('messages.failure_added'));
    }
    ...
    public function update(TemperatureRequest $request, $toilet_id, $id) {
        ...
        if ($res->save()) {
            $this->event_log($res->toJson());
            return ErrorCodeUtils::jsonResponse(ErrorCodeUtils::SUCCESS, ['messages' => trans('messages.relative_humidity_sensor') . $res->name . trans('messages.successfully_updated')]);
        }

        return ErrorCodeUtils::jsonResponse(ErrorCodeUtils::FAILURE, null, trans('messages.relative_humidity_sensor') . $res->name . trans('messages.failure_updated'));
    }
    ...
    public function destroy($toilet_id, $id) {
        ...
        if ($res->delete()) {
            $this->event_log($res->toJson());
            return ErrorCodeUtils::jsonResponse(ErrorCodeUtils::SUCCESS, ['messages' => trans('messages.relative_humidity_sensor') . trans('messages.successfully_deleted')]);
        }
        return ErrorCodeUtils::jsonResponse(ErrorCodeUtils::FAILURE, null, trans('messages.relative_humidity_sensor') . trans('messages.failure_deleted'));
    }
        ...
        try {
            ...
            foreach ($validated['data'] as $data) {
                $log = RelativeHumidityLog::create([
                    'relative_humidity_sensor_id' => $data['id'],
                    'raw_data'                    => $data['data'],
                ]);

                ProcessRelativeHumidityLog
                    ::dispatch($log)
                    ->onQueue('relative_humidity')
                    ->afterCommit();
                ...
                $report = RelativeHumidityDailyReport
                    ::firstOrNew([
                        ...
                        'relative_humidity_sensor_id' => $log->relative_humidity_sensor_id,
                    ]);
                $report->is_active = 1;
                $report->save();
            }
