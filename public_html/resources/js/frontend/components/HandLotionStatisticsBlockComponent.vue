<template>
  <v-section-box
    :title="$t('frontend_hand_lotion_use_status')"
    :hasContent="hasTodayContent"
  >
    <template #section-content>
      <div class="hand-lotion-block">
        <div class="usage-wrapper">
          <div class="usage-item">
            <span>{{ $t("hand_lotion_total") }}</span>
            <h1 class="text-fill">
              {{ (fullCount ?? 0) + (emptyCount ?? 0) }}
            </h1>
          </div>
          <div class="usage-item">
            <span>{{ $t("hand_lotion_empty_num") }}</span>
            <h1 class="text-insufficient">{{ emptyCount }}</h1>
          </div>
        </div>
        <hr />
        <div class="empty-sensor-info">
          ...
          <div class="sensor-list">
            <table>
              ...
              <tbody>
                <tr
                  v-for="(empty_sensor, index) in emptySensors"
                  :key="empty_sensor.id"
                >
                  <td>{{ index + 1 }}</td>
        ...
        <MDBTooltip direction="top" class="chart-info" v-model="tooltip_chart">
          ...
          <template #tip>
            <span
              >此數據為該廁間之洗手液感測模組透過偵測內部洗手液存量，並判斷其是否充足。</span
            >
          </template>
        </MDBTooltip>
      </div>
  ...
  <v-section-box
    :title="$t('frontend_hand_lotion_refill_record')"
    :hasContent="hasRefillContent"
  >
    <template #section-content>
      <div class="table-block">
        <div
          :class="{ 'table-control': true, disabled: refillPageIndex == 0 }"
          @click="handleRefillPrevClick()"
        >
          <svg
            width="17"
            height="29"
        ...
        <div class="table-content">
          <table>
            ...
            <tbody>
              <tr
                v-for="(record, index) in refillRecords[refillPageIndex]"
                :key="index"
              >
...
export default {
  ...
  setup(props) {
    ...
    const fullCount = ref(null);
    const emptyCount = ref(null);
    const emptySensors = ref([]);
    ...
    const updateData = (toilet_id) => {
      fetchData(toilet_id, props.dataUrl)
        .then((res) => {
          hasTodayContent.value = res.full_count + res.empty_count != 0;

          fullCount.value = res.full_count;
          emptyCount.value = res.empty_count;
          emptySensors.value = res.empty_sensors || [];
        })
        .catch((error) => {
