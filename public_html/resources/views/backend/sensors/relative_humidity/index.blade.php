...
@section('content')
    <v-sensor-list ref="list" url="{{ route('relative_humidity_sensors.index.data', [$toilet->id]) }}"
        ...
        value-unit="%"
        @can('relative_humidity_sensors.create') :can-create="true" @endcan>
        <template #breadcrumb>
...
@section('modal')
    <v-sensor-modal ref="settingModal" store-url="{{ route('relative_humidity_sensors.store', $toilet->id) }}"
        :location="{{ json_encode($location) }}" :toilet="{{ json_encode($toilet) }}"
        :type-options="{{ json_encode(trans('messages.toilet_type_options')) }}"
        :sensor_name='$t("relative_humidity")'
