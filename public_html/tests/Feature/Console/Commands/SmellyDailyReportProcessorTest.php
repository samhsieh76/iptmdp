...
class SmellyDailyReportProcessorTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function setUp(): void
    {
        ...
        $this->sensors = SmellySensor::all()->random(3);
    ...
    public function test_generate_daily_report()
    {
        ...
        foreach ($this->sensors as $sensor) {
            ...
            $rawData   = bcadd($sensor->min_value, 2.0, 2);
        ...
        Artisan::call('DailyReportProcess:Smelly', ['--date' => $this->date->format('Y-m-d')]);

        $reports = SmellyDailyReport
            ::whereIn('smelly_sensor_id', $this->sensors->pluck('id'))
        ...
        foreach ($reports as $report) {
            $newLogsBySensor = $newLogs->where('smelly_sensor_id', $report->smelly_sensor_id);
