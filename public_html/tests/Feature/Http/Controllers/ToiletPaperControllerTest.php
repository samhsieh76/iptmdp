...
class ToiletPaperControllerTest extends TestCase
{
    use WithFaker;
    use RefreshDatabase;
    ...
    public function test_receive_sensor_data()
    {
        ...
        $sensors = $toilet->toiletPaperSensors->random(2);

        $data = [
            [
                'id'   => $sensors[0]->id,
                'data' => $this->faker->randomFloat(2, 0.0, 20.0),
            ],
            [
                'id'   => $sensors[1]->id,
                'data' => $this->faker->randomFloat(2, 0.0, 20.0),
        ...
        // action
        $response = $this->postJson('/api/toilet_paper/receive_sensor_data', $input);
        ...
        foreach ($input['data'] as $data) {
            $log = ToiletPaperLog
                ::where('toilet_paper_sensor_id', '=', $data['id'])
        ...
        Queue::assertPushedOn('toilet_paper', ProcessToiletPaperLog::class);
    }
