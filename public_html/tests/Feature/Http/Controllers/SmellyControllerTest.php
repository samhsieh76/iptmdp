...
class SmellyControllerTest extends TestCase
{
    use WithFaker;
    use RefreshDatabase;
    ...
    public function test_receive_sensor_data()
    {
        ...
        $sensor = $toilet->smellySensors->random();

        $data = [
            [
                'id'   => $sensor->id,
                'data' => $this->faker->randomFloat(2, 0.0, 40.0),
            ],
        ];
        ...
        // action
        $response = $this->postJson('/api/smelly/receive_sensor_data', $input);
        ...
        foreach ($input['data'] as $data) {
            $log = SmellyLog
                ::where('smelly_sensor_id', '=', $data['id'])
        ...
        Queue::assertPushedOn('smelly', ProcessSmellyLog::class);
    }
    ...
    public function test_receive_sensor_data_with_invalid_signature()
    {
        ...
        $data = [
            [
                'id'   => $sensor->id,
                'data' => $this->faker->randomFloat(2, 0.0, 50.0),
            ],
        ];
